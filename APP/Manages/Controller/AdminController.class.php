...
class AdminController extends Controller
{
    ...
    protected function _initialize()
    {
        ...
        if (!is_login()) {
            //还没登录跳转到登录页面
            $this->redirect('Manages/Pubss/login');
        }

        // 权限检测
        $current_url = MODULE_NAME . '/' . CONTROLLER_NAME . '/' . ACTION_NAME;
        if ('Manages/Index/index' !== $current_url) {

            if (!D('Manages/Group')->checkMenuAuth()) {
                $this->error('权限不足！', U('Manages/Index/index'));
            }
        }
    ...
    public function module_config()
    {
        if (IS_POST) {
            ...
            $name        = MODULE_NAME;
            $config_file = realpath(APP_PATH . $name) . '/' . D('Manages/Module')->install_file();
            if (!$config_file) {
                $this->error('配置文件不存在');
            }
            $module_config = include $config_file;

            $module_info = D('Manages/Module')->where(array('name' => $name))->find($id);
