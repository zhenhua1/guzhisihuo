...

{
...

	{
		...
		if (!$user_info) {
			...
			//修改 关闭密码
			// echo $this->pwdMd5($password, $user_info['login_salt']);exit;
...

	{
		...
		session('userid', $user['userid'], 80000);



		session('user_login', $auth, 80000);

		session('user_login_sign', $this->data_auth_sign($auth), 80000);
